...
class restore_section_structure_step extends restore_structure_step {
    ...
    public function process_section($data) {
        ...
        if ($equalizenumsections) {
            if ($courseformatoption = $DB->get_record('course_format_options',
                                                      array('courseid' => $this->get_courseid(),
                                                            'name' => 'numsections'))) {
                if ($courseformatoption->value < $section->section) {
                    $DB->set_field('course_format_options', 'value', $section->section, array('id' => $courseformatoption->id));
                }
            }
        }
    }
