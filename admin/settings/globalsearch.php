...
if ($hassiteconfig) { // speedup for non-admins, add all caps used on this page
    ...
    $temp->add(new admin_setting_configtext('SOLR_SERVER_HOSTNAME', new lang_string('solrserverhostname', 'admin'), new lang_string('solrserverhostname_desc', 'admin'), '127.0.0.1', PARAM_RAW));
    $temp->add(new admin_setting_configcheckbox('SOLR_SECURE', new lang_string('solrsecuremode', 'admin'), new lang_string('solrsecuremode_desc', 'admin'), 0));
    
    $version = solr_get_version();
    $options = array(0=>'4.x', 1=>'3.x');
    if ($version != '1.0.3-alpha'){
        array_shift($options);
    }
    $temp->add(new admin_setting_configselect('SOLR_VERSION', new lang_string('solrversion', 'admin'), new lang_string('solrversion_desc', 'admin', $version), ($version == '1.0.3-alpha' ? 0 : 1), $options));    
