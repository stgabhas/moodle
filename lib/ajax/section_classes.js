...
section_class.prototype.init_buttons = function() {
    ...
    if (this.sectionId > 0) {
        var delbutton = main.mk_button('div', main.portal.icons['delete'], main.getString('deletesectioncheck', '', this.sectionId),
                [['title', main.portal.strings['delete'] ]]);
        YAHOO.util.Event.addListener(delbutton, 'click', this.delete_button, this, true);
        commandContainer.appendChild(delbutton);
...
section_class.prototype.delete_button = function() {
    ...
    if (!confirm(main.getString('deletesectioncheck', this.sectionId))) {
        return false;
    }
    main.remove_section(this);
    main.connect('DELETE', 'class=section&sectionId='+this.sectionId);
}
...
section_class.prototype.changeId = function(newId) {
    ...
    d = this.numberDisplay.firstChild.data;
