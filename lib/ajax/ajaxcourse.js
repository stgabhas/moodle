...
main_class.prototype.remove_section = function(el) {
    var sectionEl = el.getEl();
    var parentEl = sectionEl.parentNode;
    ...
    for (var i=0; i<sectionCount; i++) {
        if (found) {
            ...
            if (i == sectionCount - 1) {
                this.sections = this.sections.slice(0, -1);
                sectionCount--;
            }

            this.sections[i-1].changeId(i-1);

            YUI().use('node', function (Y) {
                items = Y.all('.menujump option');
                items.each(function(node) {
                    value = 'section='+i;
                    if (node.get('value').match(new RegExp("section="+i))) {
                        new_value = 'section='+(i-1);
                        text = node.get('value').replace('section='+i, new_value);
                        node.set('value', text);
                    }
                });
            });

        } else if (this.sections[i] == el) {
            found = i;
        }
    }

    // Change weekdates for each section starting at the deleted one
    var divs = YAHOO.util.Selector.query('div .weekdates');
    for (div in divs) {
        if (div >= found) {
            divs[div].innerHTML = main.sectiondates[found];
            found++;
        }
    }
    ...
    parentEl.removeChild(sectionEl);
